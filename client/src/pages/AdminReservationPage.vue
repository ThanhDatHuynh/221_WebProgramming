<template>
  <div style="min-height:70%;" class="bg-8">
    ...
    <div class="d-flex flex-wrap justify-center">
      <div
        v-for="reservation in reservations.filter(
      ...
      >
        <AdminItem
          ...
          @onBlock="toggleBlockBtn(user.id)"
        />
        <div
          class="modal fade"
          :id="`modal-reservation-${reservation.id}`"
          tabindex="-1"
          role="dialog"
        ...
        <!-- <modal
          @on-close="() => onCloseReservation(reservation.id)"
          :name="`modal-reservation-${reservation.id}`"
          :headerOptions="{
        ...
        >
          <div class="modal-header">
            ...
            <v-btn
              ...
              data-dismiss="modal"
              aria-label="Close"
              @click="() => onCloseReservation(reservation.id)"
            >
              <span aria-hidden="true">&times;</span>
            </v-btn>
          </div>
...
export default {
  ...
  methods: {
    ...
    onCloseReservation(id) {
      this.$modal.hide(`modal-reservation-${id}`);
    },
    toggleDeleteBtn(id) {
    ...
    blockUser() {
      var restURL = this.users[parseInt(this.idUserBlock) - 1].block
        ? "unblock_user"
        : "block_user";
      var settings = {
    ...
    getUser() {
      ...
      $.ajax(settings).done(function(response) {
        ...
        $.ajax(settings).done(function(response) {
          ...
          a.forEach((user) => {
            if (
              Array(blockUsers).find((blockUser) => user.email == blockUser) !=
              undefined
            ) {
              user.block = true;
            } else {
